...
namespace brokenHand.Discord.Modules.Basic
{
    public class BasicModule : InteractionModuleBase<SocketInteractionContext>
    {
        ...
        {
            await RespondAsync(embed: (await _basicService.Roll("1d20" + roll)).Build(), ephemeral: secret);
        }

        [SlashCommand("char-roll", "Roll (or calculate) something, including references to a character's values")]
        public async Task CharRoll( string roll, int? charId = null, bool secret = false)
        {

            await RespondAsync(embed: (await _basicService.CharRoll(roll, Context.User.Id, charId)).Build(), ephemeral: secret);
        }
    }
}
