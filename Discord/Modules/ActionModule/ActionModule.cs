...
namespace brokenHand.Discord.Modules.ActionModule
{
    public class ActionModule : InteractionModuleBase<SocketInteractionContext>
    {
        ...
        public async Task CharAbility(string charId, string abilityShortcut, string? targets = null, bool targetShortcuts = false)
        {
            List<EmbedBuilder> embeds = await _actionService.CharAbility(charId, abilityShortcut, targets, targetShortcuts);
        ...
        public async Task Ability(string shortcut, string? targets = null, bool targetShortcuts = false)
        {
            List<EmbedBuilder> embeds = await _actionService.Ability(Context.User.Id, shortcut, targets, targetShortcuts);
            await RespondAsync(embed: embeds.First().Build());
            embeds.RemoveAt(0);
